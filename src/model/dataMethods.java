...
public class dataMethods {
	...
	List<datos> lstNum = new ArrayList<datos>();

	public List<datos> getLstNum() {
		return lstNum;
	}

	public void setLstNum(List<datos> lstNum) {
		this.lstNum = lstNum;
	}
	
	public boolean agregarIntervalo(double intervaloX, double intervaloY, int frecuenciaSimple) {
		return lstNum.add(new datos(intervaloX, intervaloY, frecuenciaSimple));
	}
	
	public void mostrarTabla() {
		 //ESTE CODIGO SE HIZO RAPIDO POR ENDE SE IGNORO TODO TIPO DE PATRON O MEJORA DE CODIGO, ESTA HECHO PARA USO RAPIDO
		 //SE RECOMIENDA PRECAUCION AL MIRAR EL CODIGO Y LOS RESULTADOS ARROJAN "DATOS" APROXIMADOS
		...
		int auxModaAnt=0;
		int auxModaPos=0;
		datos modaAnterior = null; 
		datos modaPosterior = null; 
		double amplitud = 0;
		for(int i=0;i<lstNum.size();i++) {
